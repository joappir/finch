...
# Expose the app port (8085), the Dart VM service/DevTools port (8181),
# and the debugger's terminal WebSocket port (8282)
EXPOSE 8085 8181 8282
...
services:
  finch:
    ...
    ports:
      ...
      - "8282:8282"  # Debugger terminal WebSocket port
    environment:
...
All `FinchConfigs` values that read from `env.get(...)` (or `env.getInt(...)` / `env.getBool(...)`) can be set via Docker environment variables or a `.env` file. The example shows the pattern:
