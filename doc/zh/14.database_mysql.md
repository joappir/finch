...
Finch 使用 [mysql_client_plus](https://pub.dev/packages/mysql_client_plus) 包处理 MySQL 连接。应用启动时，`FinchApp` 会根据 `FinchMysqlConfig` 配置自动建立连接，并通过 `app.mysqlDriver` 以 `DatabaseDriver<MySQLConnectionPool>` 的形式暴露出来。
