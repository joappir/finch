...
Finch gebruikt het [mysql_client_plus](https://pub.dev/packages/mysql_client_plus)-pakket voor MySQL en biedt een laag in drie onderdelen: **MTable/MField** voor schemadefinitie, **Sqler** voor het samenstellen van queries en **DatabaseDriver** (blootgesteld als `DatabaseDriver<MySQLConnectionPool>` via `app.mysqlDriver`) voor uitvoering.
