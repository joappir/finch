...
Finch connects through a pool of `maxConnections` MongoDB connections (`Db.pool`) instead of a single connection, so concurrent requests don't contend for the same socket.
